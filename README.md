...
| `visitor_id.format_regex` | Format accepted from a client-supplied `X-Visitor-Id`/`visitor_id` — a UUID by default (`TokenResolver::generate()` produces one too, so every token in the system shares one format regardless of origin). |
