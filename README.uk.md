...
Беакон зберігає `visitor_id` у `localStorage` (мовчки не спрацьовує, якщо недоступний) і шле його як `X-Visitor-Id`, що має пріоритет над cookie на сервері — саме це дозволяє йому працювати між origin'ами, де cookie ненадійні.
...
Пріоритет при резолюції токена відвідувача на кожному запиті: клієнтський токен (header `X-Visitor-Id` чи input `visitor_id`) → наявна cookie → щойно згенерований. Cookie (`visits.cookie.name`, TTL 2 роки за замовчуванням) (пере)ставиться в чергу на кожному затрекованому запиті незалежно від того, який шлях резолвив токен.
