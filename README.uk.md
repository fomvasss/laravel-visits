...
Будь-який `GET`-запит через middleware-групу `web` трекається автоматично, окрім шляхів, що збігаються з `visits.exclude_paths` (шляхи admin/debugbar/horizon/health-check/storage виключені за замовчуванням) — і власних шляхів дашборду/whoami пакета, які завжди виключені незалежно від `exclude_paths` (інакше перегляд `/visits` сам генерував би рядки page-view про перегляд дашборду).
...
Недоступна на production (реєструється в будь-якому оточенні, крім `production`) — генерує реалістичні ланцюжки Visitor → Session → Event з узгодженими гео/девайс/UTM даними, потім запускає `visits:aggregate` по засіяному проміжку, щоб дашборд не був порожнім. `--fresh` спершу очищає наявні таблиці `visit_*` (питає підтвердження, якщо не передано ще й `--force`).
